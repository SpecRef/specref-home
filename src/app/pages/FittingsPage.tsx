...
import { FITTINGS_DB, SCHEDULES, type ScheduleKey } from "../../data/fittingData";
...
type HistoryEntry = { fittingKey: string; npsKey: string; schedule: ScheduleKey };

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Schedules that have at least one non-null value for this fitting × NPS */
function availableSchedules(fittingKey: string, npsKey: string): ScheduleKey[] {
  const fitting = FITTINGS_DB[fittingKey];
  if (!fitting) return [];
  return SCHEDULES.filter(
    s => fitting.wallThickness_mm[npsKey]?.[s] != null || fitting.weight_kg[npsKey]?.[s] != null
  );
}
...
function runLookup(
  ...
  const wt = fitting.wallThickness_mm[npsKey]?.[schedule] ?? null;
...
export default function FittingsPage() {
  const fittingKeys = Object.keys(FITTINGS_DB);

  const [selectedFitting, setSelectedFitting] = useState(fittingKeys[0]);
  const [selectedNPS,     setSelectedNPS]     = useState("4");
  const [selectedSch,     setSelectedSch]     = useState<ScheduleKey>("STD");
  ...
  useEffect(() => {
    const first = availableSchedules(selectedFitting, selectedNPS)[0] ?? "STD";
  ...
  function runQuery() {
    const data = runLookup(selectedFitting, selectedNPS, selectedSch);
    if (!data || (data.wallThickness_mm == null && data.weight_kg == null)) {
      setResult(null);
      setNoData(true);
    } else {
      setResult(data);
      ...
      setHistory(prev => {
        const filtered = prev.filter(
          h => !(h.fittingKey === selectedFitting && h.npsKey === selectedNPS && h.schedule === selectedSch)
        );
        return [
          { fittingKey: selectedFitting, npsKey: selectedNPS, schedule: selectedSch },
          ...filtered,
        ].slice(0, 5);
      });
    }
  ...
  function restoreFromHistory(entry: HistoryEntry) {
    ...
    const data = runLookup(entry.fittingKey, entry.npsKey, entry.schedule);
    const valid = data && (data.wallThickness_mm != null || data.weight_kg != null);
    setResult(valid ? data : null);
    setNoData(!valid);
  }

  const currentFitting = FITTINGS_DB[selectedFitting];
      ...
      <div className="relative z-10">
        ...
        <section className="mx-auto max-w-7xl px-6 pb-24 grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-800/40 backdrop-blur-md p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    {fittingKeys.map(k => (
                      <option key={k} value={k}>{FITTINGS_DB[k].label}</option>
                    ))}
                  </select>
                </div>
                  ...
                  >
                    {npsKeys.map(k => (
                      <option key={k} value={k}>
                        NPS {FITTINGS_DB[selectedFitting].sizes[k].nps}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={selectedSch}
                    onChange={e => setSelectedSch(e.target.value as ScheduleKey)}
            ...
            {/* Fitting Info Card */}
            <div className="rounded-2xl border border-white/10 bg-slate-800/40 backdrop-blur-md p-6">
              <h3 className="text-white mb-3 text-sm uppercase tracking-wider">Fitting Info</h3>
                ...
                {currentSize && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{currentSize.od_mm} mm</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Schedules</span>
                      <span className="text-amber-300">{schedules.length} available</span>
                    </div>
                  </>
                )}
              </div>
                ...
                <ul className="space-y-2">
                  {history.map((h, i) => (
                    <li key={i}>
                      ...
                      >
                        {FITTINGS_DB[h.fittingKey]?.label} · NPS {FITTINGS_DB[h.fittingKey]?.sizes[h.npsKey]?.nps} · {h.schedule}
                      </button>
                    </li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-2">
            ...
            {!result && !noData && (
              <div className="rounded-2xl border border-white/10 bg-slate-800/25 backdrop-blur-md p-12 flex flex-col items-center justify-center text-center min-h-64">
                ...
                <p className="text-slate-300 mb-1">Select parameters and click <span className="text-amber-300">Get Data</span></p>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-amber-400/20 bg-amber-500/5 backdrop-blur-md p-6">
                  <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
                    <h3 className="text-white">
                      NPS {result.nps}
                      <span className="text-slate-400 text-base ml-2">— {result.fittingLabel}</span>
                    </h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="rounded-xl border border-white/10 bg-slate-800/40 backdrop-blur-md p-5">
                    <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Outside Diameter</p>
                    <p className="text-2xl text-white font-light">{result.od_mm} mm</p>
                    <p className="text-sm text-slate-400 mt-1">{(result.od_mm / 25.4).toFixed(3)}"</p>
                  </div>

                  {/* Through-length */}
                  <div className="rounded-xl border border-white/10 bg-slate-800/40 backdrop-blur-md p-5">
                    <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Through Length</p>
                    <p className="text-2xl text-white font-light">{result.length_mm} mm</p>
                    <p className="text-sm text-slate-400 mt-1">{(result.length_mm / 25.4).toFixed(3)}"</p>
                  </div>

                  {/* Wall Thickness */}
                  <div className="rounded-xl border border-white/10 bg-slate-800/40 backdrop-blur-md p-5">
                    <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Wall Thickness</p>
                    <p className="text-2xl text-white font-light">
                      {result.wallThickness_mm != null ? `${result.wallThickness_mm} mm` : "—"}
                    </p>
                    <p className="text-sm text-slate-400 mt-1">
                      {result.wallThickness_mm != null
                        ? `${(result.wallThickness_mm / 25.4).toFixed(4)}"`
                        : "Not tabulated"}
                    </p>
                  </div>

                  {/* Weight — full width */}
                  <div className="rounded-xl border border-amber-400/10 bg-amber-500/5 backdrop-blur-md p-5">
                    <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Approx. Weight</p>
                    <p className="text-3xl text-amber-300 font-light">
                      {result.weight_kg != null ? `${result.weight_kg} kg` : "—"}
                    </p>
                    <p className="text-sm text-slate-400 mt-1">
